...
```
        **Frequently used entry.attribute:**
        ...
        self.point                        #list of all (x,y,z) sites' wyckoff positions in CIF
...
- `get_non_elements(entry, excluded_elements=None)`:
    ...
    In xxx file, we exclude the following noble gases and radioactive elements: ['He', 'Ne', 'Ar', 'Kr', 'Es'].
...
- Outliers documents:
    ...
    - `intersect_orb.csv`: Recording positional disordered sites. Two sites are intersect if distance between sites < thresh hold, refer to: [Exploration of positional disorder](https://www.notion.so/Exploration-of-positional-disorder-277368b04661808f9f59e25207e3a437?pvs=21)
    ...
    - `main_err.json`: The `disorder_label` fail to process `ICSDEntry`. Normally should be empty. Contact me if occur.
...
- `wyck_float_warn.csv` : suggests a **poor XRD refinement quality**. The refined coordinates in CIF being shifted from their standard Wyckoff values (appear as    floating-point error).
- `occ_err_sites.csv`: occypancy error, will fail in pymatgen
- `equivalent_but_distinct_sites.csv` : Wyckoff coordinates assignment is questionable, or containing floating-point error
